...
/*   Updated: 2022/04/18 17:24:59 by fbes          ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int	export_only(size_t i, t_cmd *cmd)
{
	t_envar	*envar;
...
static int	set_and_export(int i, t_cmd *cmd, char *equals_pos)
{
	*equals_pos = '\0';
...
int	mini_export(int is_child, t_cmd *cmd)
{
	...
	while (cmd->params[i])
	{
		...
		else if (!is_valid_env_name(cmd->params[i]) && is_child)
			export_err(cmd->params[i]);
		else if (equals_pos && !set_and_export(i, cmd, equals_pos))
			return (ENOMEM);
		else if (!export_only(i, cmd))
			return (ENOMEM);
		i++;
	}
	return (0);
